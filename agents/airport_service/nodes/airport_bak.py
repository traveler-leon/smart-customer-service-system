    ...
    kb_prompt = ChatPromptTemplate.from_messages([
        ...
        ("human", 
         """
            ...
            这是必须使用的短语: "抱歉，我暂时无法提供这方面的信息。请您参考国家法律、行政法规、规章规定"
            ...
            <instructions> 
                ...
                - **步骤 2: 需要澄清**
                - 如果 <context> 包含相关信息，但这些信息根据不同条件有差异的，而用户问题未明确这些条件（用户问题粒度粗或者太宽，而上下文粒度细或规定更细），必须像用户提出一个澄清问题。
...
""")
    ...
    if len(context_docs) < max_msg_len:
        ...
        return {"messages":"抱歉，我暂时无法提供这方面的信息。请您参考国家法律、行政法规、规章规定"}
    else:
