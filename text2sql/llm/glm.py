...
class GlmLLM(AsyncLLMProvider):
    """智普AI异步LLM实现
    
    基于OpenAI兼容接口的智普大语言模型
    """
    ...
    def __init__(self, config=None):
        ...
        self.max_tokens = self.config.get("max_tokens", 120000)
        self.client = None
        
        logger.info(f"初始化智普LLM提供者，模型: {self.model}")
    ...
    def _ensure_client(self):
        ...
        if self.client is None:
            ...
            logger.debug("智普AI异步客户端已初始化")
    
    async def close(self):
        """关闭异步会话"""
        # AsyncOpenAI 客户端会自动管理会话，不需要手动关闭
        self.client = None
        logger.debug("智普AI异步客户端已重置")
