...
class OpenrouterLLM(AsyncLLMProvider):
    """OpenRouter异步LLM实现
    
    基于OpenAI兼容接口的深度求索大语言模型
    """
    ...
    def __init__(self, config=None):
        ...
        self.model = self.config.get("model", "anthropic/claude-3.7-sonnet")
        self.max_tokens = self.config.get("max_tokens", 64000)
        self.client = None
        
        logger.info(f"初始化OpenRouter LLM提供者，模型: {self.model}")
    ...
    def _ensure_client(self):
        ...
        if self.client is None:
            ...
            logger.debug("OpenRouter异步客户端已初始化")
    
    async def close(self):
        """关闭异步会话"""
        # AsyncOpenAI 客户端会自动管理会话，不需要手动关闭
        self.client = None
        logger.debug("OpenRouter异步客户端已重置")
